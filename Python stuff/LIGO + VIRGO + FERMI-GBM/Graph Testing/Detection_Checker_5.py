...
for q in range(0,iterations):
    ...
    for a in range(0,trials):
        ...
        h.append(((((170)**2))/(distance[datacounter] ** 2)))
    ...
    for b in range(0,trials):
        if thetaobs[listnumt_GRB] <= math.radians(10) and thetaobs[listnumt_GRB] >= -1 * math.radians(10):
            FOn = (energy * (1-math.cos(math.radians(thetaj))))/(( (4/3) * math.pi*((((distance[listnumt_GRB]/2.2435) ** 3)))))
            FOn = FOn/(9.521 * (10**48))
            fluence.append(FOn)
            ...
            FOnn = (energy * (1-math.cos(math.radians(thetaj))))/(( (4/3) * math.pi*((((distance[listnumt_GRB]/2.2435) ** 3)))))
            FOff = eta * FOnn 
    ...
    GWGRBFINALnum = GWGRBFINALnum + GW_GRB_TEST
...
print('GW Volume '+ str((((450/2.25)**3) * math.pi * (4/3) * GWPERCENT)))
print('GRB Volume '+ str((((450/2.25)**3) * math.pi * (4/3) * GRBPERCENT)))
print('GWGRB Volume '+ str((((450/2.25)**3) * math.pi * (4/3) * GWGRBPERCENT)))
