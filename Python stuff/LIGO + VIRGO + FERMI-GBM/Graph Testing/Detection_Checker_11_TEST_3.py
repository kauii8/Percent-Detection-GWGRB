...
iterations = 100
GRBFINALnum_off, GRBFINALnum_struc_sim, GRBFINALnum_struc_best, GWFINALnum, GWGRBFINALnum_off, GWGRBFINALnum_struc_sim, GWGRBFINALnum_struc_best = 0, 0, 0, 0, 0, 0, 0
GWPERCENTMEAN, GRBPERCENTMEAN_struc_sim, GRBPERCENTMEAN_struc_best, GRBPERCENTMEAN_off, GWGRBPERCENTMEAN_struc_sim, GWGRBPERCENTMEAN_struc_best = [], [], [], [], [], [], []
for q in range(0,iterations):
    ...
    datacounter, GRBTESTnum_off, GRBTESTnum_struc_sim, GRBTESTnum_struc_best, GWTEST = 0, 0, 0, 0, 0
    ...
    SNRcalculated, SNRnum, GWTEST, GWTESTnum = [], 0, [], 0
    for f in range(0, trials):
        ...
        if SNRcalculated[f] >= 64:
            ...
            GWTEST.append(False)
    '''------SNR CALCULATOR/CHECKER------'''
    '''------STRUCTURED JETS BEST FIT------'''
    fluence_struc_best, fluenceonoff, GRBTEST_struc_best, energy = [], [], [], []
    energyorig = 7e51
    thetaj = 10
    alpha = 1.9
    thetac = 9
    ...
    for a in range(0,trials):
        energy.append(angletoenergy(thetaobs[a], energyorig, thetac, alpha))
        fluence_struc_best.append(energy[a]/(4 * math.pi * ((distance[a] * 3.086e26) ** 2)))
    '''------STRUCTURED JETS BEST FIT------'''
    '''------STRUCTURED JETS GENERATION SIMULATION DATA------'''
    energyorig = 7e51
    ...
    for p in range(0,trials):
        fluence_struc_sim.append(real_energy_list[p]/(4 * math.pi * ((distance[p] * 3.086e26) ** 2)))
        '''------STRUCTURED JETS GENERATION SIMULATION DATA------'''
    '''------OFF AXIS------'''
    fluence_off, fluenceonoff, GRBTEST_off = [], [], []
    gamma = 100
    energyinitial = 7e51
    ...
    GRBFINALnum_struc_best = GRBFINALnum_struc_sim + GRBTESTnum_struc_sim
    GWGRBFINALnum_off = GWGRBFINALnum + GWGRBTESTnum_off
    GWGRBFINALnum_struc_sim = GWGRBFINALnum + GWGRBTESTnum_struc_sim
    GWGRBFINALnum_struc_best = GWGRBFINALnum + GWGRBTESTnum_struc_best
...
'''------DATA DISPLAY------'''
print(' ')
print('GW PERCENT ' + str(GWPERCENT) + '        GRB PERCENT ' + str(GRBPERCENT_off) + '        GWGRB PERCENT ' + str(GWGRBPERCENT_off))
print(' ')
print('GW ' + str(GWFINALnum) + '        GRB ' + str(GRBFINALnum_off) + '        GWGRB ' + str(GWGRBFINALnum) + '        TOTAL ' + str(iterations * trials))
print(' ')
print('GW STANDARD DEVIATION ' + str(GWSD) + '        GRB STANDARD DEVIATION ' + str(GRBSD_off) + '        GWGRB STANDARD DEVIATION ' + str(GWGRBSD_off))
print(' ')
print('GW Volume '+ str(((((450)**3) * math.pi * (4/3) * GWPERCENT)/(1000**3))) + ' +/- ' + str(((((450)**3) * math.pi * (4/3) * GWSD)/(1000**3))))
print('GRB Volume '+ str((((450)**3) * math.pi * (4/3) * GRBPERCENT_off)/(1000**3)) + ' +/- ' + str(((((450)**3) * math.pi * (4/3) * GRBSD_off))/(1000**3)))
print('GWGRB Volume '+ str((((450)**3) * math.pi * (4/3) * GWGRBPERCENT_off)/(1000**3)) + ' +/- ' + str(((((450)**3) * math.pi * (4/3) * GWGRBSD_off))/(1000**3)))
print('All volumes in Gpc^3')
print(' ')
#print('GWGRB/GW, percent detection ' + str(100 * ((((450)**3) * math.pi * (4/3) * GWGRBPERCENT_off)/(1000**3))/(((((450)**3) * math.pi * (4/3) * GWPERCENT)/(1000**3)))))
