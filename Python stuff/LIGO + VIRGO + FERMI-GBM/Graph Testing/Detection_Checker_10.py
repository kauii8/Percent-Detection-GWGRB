...
epsilonE, epsilonB, pe, nism, Rvol = .02, .002, 2/13, 1.2e-5, (10e50/(1.7 * (299792458 ** 2)))
...
for q in range(0,iterations):
    ...
    for b in range(0,trials):
        ...
        initiallorentz = lorentzc/ (1 + ((theta[listnumt_GRB]/thetac) ** 2 ))
        energyf52 = .022 * (epsilonB ** (-3/5)) * (epsilonE ** (-3/5)) * (energy ** (4/5)) * ((initiallorentz/100) ** (-4/5)) * (nism ** (-2/5)) 
        '''
        lorentzmaterial.append((math.fabs(math.sin(math.radians((thetaobs[listnumt_GRB]/2)))) ** (1/7)) * 17400 )
        tobs.append(Rvol/((lorentzmaterial[listnumt_GRB] ** 2) * 4 * 299792458 ))
        ...
        if thetaobs[listnumt_GRB] <= math.radians(5) and thetaobs[listnumt_GRB] >= -1 * math.radians(5):
            ...
            '''------STRUCTURED JETS------'''
            Fvmax = 1.1 * 10e5 * (epsilonB ** (1/2)) * (energyf52/(10 **52)) * (nism ** (1/2)) * ((distance[listnumt_GRB] * 9.521 * (10**23.6))/(10 ** 28)) * 10
            fluence.append(Fvmax)
            '''------STRUCTURED JETS------'''
            '''------OFF AXIS------'''
            '''
