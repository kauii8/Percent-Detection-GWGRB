...
for q in range(0,iterations):
    ...
    for z in range(0,trials):
        ...
        h_LOUIS.append(((((Dv_LOUIS * 2.25 * 3.086e24)**2))/(((distance[z] * 3.086e24) ** 2))))
        h_WASH.append(((((Dv_WASH * 2.25 * 3.086e24)**2))/((distance[z] * 3.086e24) ** 2)))
        h_VIRGO.append(((((Dv_VIRGO * 2.25 * 3.086e24)**2))/((distance[z] * 3.086e24) ** 2)))
    ...
    for a in range(0,trials):
        ...
        if thetaobs[a] <= math.radians(5) or thetaobs[a] > math.radians(175):
            fluence_off.append(FOn)
        else:
            deltaobs = 1/(gamma * (1 - beta * math.cos(math.radians(thetaobs[a] - math.radians(thetaj)))))
            deltazero = 1/(gamma * (1 - beta * math.cos(math.radians(0 - math.radians(thetaj)))))
            eta = (deltaobs/deltazero) 
            fluence_off.append(eta * FOn)
            '''
            eta = delta_function(gamma, beta, thetaobs[a], math.radians(thetaj))/delta_function(gamma, beta, 0, thetaj)
            FOff = eta * FOn
            fluence_off.append(FOff)
            '''
    ...
    for h in range(0, len(thetaobs)):
        ...
        if math.degrees(thetaobs[h]) > 38.49856821:
            real_energy_list.append(angletoenergy(thetaobs[h], energyinitial, math.radians(thetac), alpha))
        else:
...
GWPERCENT = GWFINALnum / float(iterations * trials)
GWSD = standarddev(GWPERCENTMEAN, GWPERCENT, iterations)

GRBPERCENT_off = GRBFINALnum_off/ float(iterations * trials)
GWGRBPERCENT_off = GWGRBFINALnum_off/ float(iterations * trials)
...
GRBPERCENT_struc_sim = GRBFINALnum_struc_sim/ float(iterations * trials)
GWGRBPERCENT_struc_sim = GWGRBFINALnum_struc_sim/ float(iterations * trials)
...
GRBPERCENT_struc_best = GRBFINALnum_struc_best/ float(iterations * trials)
GWGRBPERCENT_struc_best = GWGRBFINALnum_struc_best/ float(iterations * trials)
