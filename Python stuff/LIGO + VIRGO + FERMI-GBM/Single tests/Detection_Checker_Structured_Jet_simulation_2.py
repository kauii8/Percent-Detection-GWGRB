...
for q in range(0,iterations):
    ...
    for p in range(0,trials):
        fluence.append(real_energy_list[p]/(4 * math.pi * ((distance[p] * 3.086e26) ** 2)))
        '''------STRUCTURED JETS GENERATION------'''   
...
GRBPERCENT = GRBFINALnum/ float(iterations * trials)
GWGRBPERCENT = GWGRBFINALnum/ float(iterations * trials)
