...
def function1(phi_, theta_, pl_, ex_):
    return(float((ex_ * -1 * float(math.cos(theta_)) * math.sin(2 * phi_)) + pl_(((math.cos(theta_) * math.cos(phi_))**2) - (math.sin(phi_))**2)))
    
    
def function2(phi_, theta_, pl_, ex_):
 return(float((ex_ * 1 * math.cos(theta_) * math.sin(2 * phi_)) + pl_(((math.cos(theta_) * math.sin(phi_))**2) - (math.cos(phi_))**2)))

def function3(phi_, theta_, mult_, pl_, ex_):
    return(float((((function1(phi_, theta_, pl_, ex_)) * (1 + cmath.exp(-1j * math.pi * math.sin(theta_) * math.cos(phi_))) * 1j * math.sin(theta_) * math.cos(phi_))
    /(math.pi * (1 - (math.sin(theta_) * math.cos(phi_))**2)))) - 
    float((((function2(phi_, theta_, pl_, ex_)) * (1 + cmath.exp(-1j * math.pi * math.sin(theta_) * math.sin(phi_))) * 1j * math.sin(theta_) * math.sin(phi_))
    /(math.pi * (1 - (math.sin(theta_) * math.sin(phi_))**2)))))
   
'''   
   float(-1(((function2(phi_, theta_, pl_, ex_)) * (1 + cmath.exp(-1j * math.pi * math.sin(theta_) * math.sin(phi_))) * 1j * math.sin(theta_) * math.sin(phi_))
    /(math.pi * (1 - (math.sin(theta_) * math.sin(phi_))**2)))))
'''
